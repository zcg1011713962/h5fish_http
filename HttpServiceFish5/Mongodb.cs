...
class MongodbPayment : MongodbHelper<MongodbPayment>
{
    ...
    protected override void init_table()
    {
        addIndex(PayTable.QUICKSDK_PAY);
        addIndex(PayTable.DUOYOU_PAY);
        addIndex(PayTable.ALI_WEB_PAY);
        addIndex("weixin_web_pay");
        addIndex("huawei_pay");
        addIndex(PayTable.OPPO_PAY);
    }
    ...
    void addIndex(string tableName)
    {
        ...
        indexes = new string[] { "OrderID", "Process" };
        if (!tmp.IndexExists(indexes))
            tmp.CreateIndex(indexes);
    }
}
