...
class MongodbPayment : MongodbHelper<MongodbPayment>
{
    ...
    protected override void init_table()
    {
        ...
        //beecloud
        tmp = mMongodbClient.GetCollection("beecloud_pay");
        indexes = new string[] { "Account", "OrderID", "Process" };
        if (!tmp.IndexExists(indexes))
            tmp.CreateIndex(indexes);
        ...
        //th_pay
        tmp = mMongodbClient.GetCollection("th_pay");
        if (!tmp.IndexExists("orderid"))
            tmp.CreateIndex("orderid");

        if (!tmp.IndexExists("userid"))
            tmp.CreateIndex("userid");
    }
}
