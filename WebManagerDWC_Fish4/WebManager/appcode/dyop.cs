        ...
        if (p.m_op == 3 || p.m_op == 4 || p.m_op == 5)
        {
            ...
            if (res) 
            {
                ...
                else if (p.m_op == 4)
                {
                    ...
                    data.Add("JsckpotSmallPump", p.m_ratio);
                }
...
{
    private string[] m_playerFields = { "itemName", "phone", "genTime", "playerId" };
    public override OpRes doDyop(object param, GMUser user)
    {
...
public class DyOpFishlordParamAdjustNew : DyOpParamAdjust
{
    ...
    OpRes calParamInner(ParamFishlord5 param, ParamInner result)
    {
        ...
        /////////////////////////////////////
        if (result.m_roomId == 2 || result.m_roomId == 3 || result.m_roomId == 5 || result.m_roomId == 6)
        {
...
public class DyOpOperationGameCtrl : DyOpBase 
{
    ...
    public override OpRes doDyop(object param, GMUser user)
    {
        ...
        if (p.m_op == 0)
        {
            if (string.IsNullOrEmpty(p.m_channelNo) || string.IsNullOrEmpty(p.m_param))
                return OpRes.op_res_param_not_valid;
        }
    ...
    private OpRes addVerControl(ParamQuery p, GMUser user)
    {
        ...
        if (p.m_op == 0) //添加
        {
            data.Add("channel", p.m_channelNo);
    ...
    public void viewGameVersionCtrl(GMUser user)
    {
        ...
        List<Dictionary<string, object>> dataList =
            DBMgr.getInstance().executeQuery(TableName.OP_CHANNEL_VER_CONTROL, serverId, DbName.DB_ACCOUNT);
        m_ctrlList.Clear();
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            if (data.ContainsKey("channel"))
                item.m_channelNo = Convert.ToString(data["channel"]);
