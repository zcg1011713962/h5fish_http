...
public class CDBMgr<T> where T : CDBMgr<T>, new()
{
    ...
    protected CDBServer create(string url, DbServerInfo dbInfo, bool special = false)
    {
        ...
        {
            LOGW.Info("CDBMgr,连接数据库 {0} 时发生异常，异常信息{1}!", url, ex.ToString());
        }
        return null;
    }
    ...
    // 更新数据表中某些字段值
    public bool update(string tablename, UpdateBuilder data, IMongoQuery query, int serverid, int dbid, bool noAutoInsert = false)
    {
        if (data == null)
            return false;
        try
    ...
    public bool keyExists(string table, IMongoQuery imq, int serverid, int dbid)
    {
        try
        {
            return m_mongoServer[serverid].getDB(dbid).ExecuteGetCount(table, imq) > 0;
        }
        catch (System.Exception ex)
        {
            LOGW.Info(ex.ToString());
...
//                 tmp.CreateIndex(indexname);
    ...
    public bool ExecuteUpdateByQuery(string table, IMongoQuery queries, UpdateBuilder ub, UpdateFlags flags = UpdateFlags.None)
    {
        try
        {
            var cb = check_table(table);
...
public class CDBServer
{
    ...
    public bool init<T>(string url, DbServerInfo dbInfo, CDBMgr<T> dbMgr, bool special = false) where T : CDBMgr<T>, new()
    {
        ...
        if (dbMgr.isAccountDb(url))
        {
          ...
          // m_client[DbName.DB_CONFIG].MongoDb = create(m_dbName[DbName.DB_CONFIG], tmpURL);
            ...
            createPLDB(m_client, "", "", tmpURL);
        }
        else // 其他游戏相关数据库
        {
            string tmpURL = dbMgr.getAccountDbURL();
            createPLDB(m_client, url, dbInfo.m_logDbIp, tmpURL);
            createPLDB(m_clientSlave, dbInfo.m_playerDbSlave, dbInfo.m_logDbSlave, tmpURL);
        }
    ...
    void createPLDB(CMongodbHelper[] client, string playerURL, string logURL, string accountURL)
    {
        if(!string.IsNullOrEmpty(accountURL))
        {
            m_client[DbName.DB_ACCOUNT] = new CMongodbHelper();
            m_client[DbName.DB_ACCOUNT].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_ACCOUNT], accountURL);// create(m_dbName[DbName.DB_ACCOUNT], tmpURL);

            // 充值数据库只连接这个
            m_client[DbName.DB_PAYMENT] = new CMongodbHelper();
            m_client[DbName.DB_PAYMENT].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_PAYMENT], accountURL);//create(m_dbName[DbName.DB_PAYMENT], tmpURL);

            m_client[DbName.DB_CONFIG] = new CMongodbHelper();
            m_client[DbName.DB_CONFIG].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_CONFIG], accountURL);//create(m_dbName[DbName.DB_CONFIG], tmpURL);
        }

        if (!string.IsNullOrEmpty(playerURL))
        {
            client[DbName.DB_PLAYER] = new CMongodbHelper();
            client[DbName.DB_PLAYER].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_PLAYER], playerURL); // create(m_dbName[DbName.DB_PLAYER], playerURL);

            client[DbName.DB_GAME] = new CMongodbHelper();
            client[DbName.DB_GAME].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_GAME], playerURL);// create(m_dbName[DbName.DB_GAME], playerURL);
        }

        if (!string.IsNullOrEmpty(logURL))
        {
            client[DbName.DB_PUMP] = new CMongodbHelper();
            client[DbName.DB_PUMP].MongoDb = TConnectionMgr.getInstance().create(m_dbName[DbName.DB_PUMP], logURL);// create(m_dbName[DbName.DB_PUMP], logURL);
        }
    }
    ...
    private MongoDatabase create(string dbname, string pools)
    {
        ...
        {
            LOGW.Info("CDBServer, 连接数据库 {0} 时发生异常, {1}", pools, ex.Message);
        }
        return null;
    }
...
class IndexPump : IndexBase
{
    public override void createIndex(CMongodbHelper db, string url)
    {
        ...
        try
        {
            ...
            var table2 = mdb.GetCollection(TableName.STAT_PUMP_PLAYER_ACTIVITY_TURRET);
            table2.CreateIndex(IndexKeys.Ascending("playerId"), IndexOptions.SetBackground(true));
            table2.CreateIndex(IndexKeys.Ascending("day"), IndexOptions.SetBackground(true));
        }
        catch (System.Exception ex)
        {	
        }
...
{
    static TConnectionMgr s_inst = new TConnectionMgr();

    Dictionary<string, MongoClient> m_conn = new Dictionary<string, MongoClient>();

    public static TConnectionMgr getInstance()
    {
        return s_inst;
    }

    public MongoDatabase create(string dbname, string pools)
    {
        try
        {
            bool has = hasPort(pools);
            if (!has)
            {
                pools = pools + ":27017";
            }
            MongoClient client = null;
            if (m_conn.ContainsKey(pools))
            {
                client = m_conn[pools];
            }
            else
        ...
        {
            LOGW.Info("TConnectionMgr,连接数据库 {0} 时发生异常, {1}", pools, ex.Message);
        }
        return null;
    }

    private bool hasPort(string pools)
    {
        int index = pools.IndexOf(":");
        return index >= 0;
    }
}
