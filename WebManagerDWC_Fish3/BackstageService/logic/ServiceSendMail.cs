...
public class ServiceSendMail : ServiceBase
{
    ...
    private void sendMail(ParamSendMailFullSvr sendParam)
    {
        ...
        LogMgr.info("开始群发邮件 {0}", imq.ToString());
        ...
        while (true)
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                // 标识是系统发送的邮件
                tmp.Add("senderId", 0);
                tmp.Add("mainReason", (int)PropertyReasonType.type_reason_receive_gm_mail);
        ...
        LogMgr.info("结束群发邮件 {0}", imq.ToString());
    }
    ...
    private IMongoQuery createImq(ParamSendMailFullSvr sendParam)
    {
        ...
        if (sendParam.m_condition != null)
        {
            ...
            if (sendParam.m_condition.ContainsKey("channel"))
            {
                int channelId = 0;
                if (int.TryParse(Convert.ToString(sendParam.m_condition["channel"]), out channelId))
                {
                    queryList.Add(Query.EQ("ChannelID", channelId));
                }
                else
                {
                    LogMgr.error("sendMail，channel not int");
                }
            }

            queryList.Add(Query.EQ("is_robot", false));
