...
public class DBMgr
{
    ...
    private void open()
    {
        ...
        if (!m_dbServer.ContainsKey(acc))
        {
            ...
            m_mongoServer[i] = create(acc, null); // null;
            i++;
        }
        if (!m_dbServer.ContainsKey(pay))
        {
            m_dbServer.Add(pay, i);
            m_mongoServer[i] = create(pay, null); //null;
            i++;
        }
    }
    ...
    public int getDbId(string pools, bool special = false)
    {
        if (m_dbServer.ContainsKey(pools))
        {
            ...
            if (m_mongoServer[index] == null)
            {
                ...
                if (m_mongoServer[index] == null)
                {
                    LogMgr.info("getDbId函数，无法联接数据库!");
                    return -1;
                }
            }
            return index;
        }
        LogMgr.info("没有找到地址:{0}，将采用原地址", pools);
        return -1;
    }
...
public class DBServer
{
    ...
    void createAll(string url, DbServerInfo dbInfo)
    {
        XmlConfig xml = ResMgr.getInstance().getRes("dbserver.xml");
        string acc = xml.getString("account", "");
        string pay = xml.getString("payment", "");
        ...
        m_client[DbName.DB_ACCOUNT] = new MongodbHelper();
        m_client[DbName.DB_ACCOUNT].MongoDb = ConnectionMgr.getInstance().create(m_dbName[DbName.DB_ACCOUNT], acc);

        m_client[DbName.DB_PAYMENT] = new MongodbHelper();
        m_client[DbName.DB_PAYMENT].MongoDb = ConnectionMgr.getInstance().create(m_dbName[DbName.DB_PAYMENT], pay);
        createOther(url, dbInfo);
    }
    ...
    void createOther(string url, DbServerInfo dbInfo)
    {
        if (dbInfo != null)
        {
            ...
            m_client[DbName.DB_PLAYER].MongoDb = ConnectionMgr.getInstance().create(m_dbName[DbName.DB_PLAYER], dbInfo.m_serverIp);

            m_client[DbName.DB_PUMP] = new MongodbHelper();
            m_client[DbName.DB_PUMP].MongoDb = ConnectionMgr.getInstance().create(m_dbName[DbName.DB_PUMP], dbInfo.m_logDbIp);

            m_client[DbName.DB_GAME] = new MongodbHelper();
            m_client[DbName.DB_GAME].MongoDb = ConnectionMgr.getInstance().create(m_dbName[DbName.DB_GAME], dbInfo.m_serverIp);
        }
    }
    ...
    public MongodbHelper getDB(int index)
    {
        ...
        {
            LogMgr.error("获取某数据库服务器上某个db时出错，索引超出范围, index = {0}", index);
            return null;
        }
    ...
    private MongoDatabase create(string dbname, string pools)
    {
        ...
        {
            LogMgr.error("连接数据库 {0} 时发生异常, {1}", pools, ex.Message);
        }
        return null;
    }
...
class ConnectionMgr
{
    static ConnectionMgr s_inst = new ConnectionMgr();
