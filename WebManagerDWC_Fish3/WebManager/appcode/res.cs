...
{
    public string m_redisName;
    public string m_redisIP;
}
...
public class ResMgr
{
    ...
    public Dictionary<string, RedisInfo> m_redisInfo = new Dictionary<string, RedisInfo>();
    ...
    public RedisInfo getRedisInfo(string name)
    {
        if (m_redisInfo.ContainsKey(name))
        {
            return m_redisInfo[name];
        }
    ...
    private void setUpDbServerInfo()
    {
        ...
        for (int i = 0; i < t.Count; i++)
        {
            ...
            if(t[i].ContainsKey("redisIp"))
            {
                info.m_redisIp = Convert.ToString(t[i]["redisIp"]);
            }
            else
            {
                info.m_redisIp = "";
    ...
    private void setUpRedis(XmlConfigMaker c)
    {
        XmlConfig cfg = loadXmlConfig("RedisCfg.xml", c, false);
        List<Dictionary<string, object>> t = cfg.getTable("server");
        for (int i = 0; i < t.Count; i++)
        {
            Dictionary<string, object> data = t[i];
            RedisInfo info = new RedisInfo();
            info.m_redisName = Convert.ToString(data["redisName"]);
            info.m_redisIP = Convert.ToString(data["redisIP"]);
            m_redisInfo.Add(info.m_redisName, info);
        }
    }
}
...
public class InsidePlayerCFG : XmlDataTable<InsidePlayerCFG, int, InsidePlayerData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "M_InsideId.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
        ...
        for (node = node.FirstChild; node != null; node = node.NextSibling)
        {
            InsidePlayerData tmp = new InsidePlayerData();
            string sid = node.Attributes["Id"].Value;
            tmp.m_id = Convert.ToInt32(sid);
            tmp.m_playerId = Convert.ToInt32(node.Attributes["PlayerId"].Value);
...
///////////////////////////////////////////////////////////////////////////
//投放
public class ChannelLaunchData
{
    public int m_id;
    public int m_lv;
    public string m_lvName = "";
    public int m_type;
    public int m_RMB;
}
public class ChannelLaunch : XmlDataTable<ChannelLaunch, int, ChannelLaunchData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "M_ChannelLaunch.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
        ...
        for (node = node.FirstChild; node != null; node = node.NextSibling)
        {
            ChannelLaunchData tmp = new ChannelLaunchData();
            string sid = node.Attributes["ID"].Value;
            tmp.m_id = Convert.ToInt32(sid);
            tmp.m_lv = Convert.ToInt32(node.Attributes["Level"].Value);
            tmp.m_lvName = node.Attributes["Name"].Value;
            tmp.m_RMB = Convert.ToInt32(node.Attributes["Reward1"].Value);
            m_data.Add(tmp.m_id, tmp);
        }
    }
}
...
{
    // db中字段名
    public string m_filedName;
    // M_ChannelLaunch中id
    public int m_id;
}
public class ChannelLaunchCFGData
{
    public int m_id;
    public List<ChannelParam> m_goldPlayer;
    public List<ChannelParam> m_payPlayer;

    public  List<ChannelParam> parase(string str, string filedPrefix)
    {
        List<ChannelParam> result = new List<ChannelParam>();
        if (str == "")
            return result;

        string[] part = str.Split('#');
        string[] filed = part[0].Split(',');
        string[] ids = part[1].Split(',');
        if(filed.Length==ids.Length)
        {
            for(int i = 0; i < filed.Length;i++)
            {
                ChannelParam t = new ChannelParam();
                t.m_id = Convert.ToInt32(ids[i]);
                t.m_filedName = filedPrefix + filed[i].Trim();
                result.Add(t);
            }
        }
        else
        {
            throw new Exception("ChannelLaunchCFGData error");
        }
        return result;
    }

    public void parseGold(string str)
    {
        List < ChannelParam > r = parase(str, "gold");
        m_goldPlayer = r;
    }

    public void parsePay(string str)
    {
        List<ChannelParam> r = parase(str, "pay");
        m_payPlayer = r;
    }
}
public class ChannelLaunchCFG : XmlDataTable<ChannelLaunchCFG, int, ChannelLaunchCFGData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "M_ChannelLaunchCFG.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
        ...
        for (node = node.FirstChild; node != null; node = node.NextSibling)
        {
            ChannelLaunchCFGData tmp = new ChannelLaunchCFGData();
            string sid = node.Attributes["ID"].Value;
            tmp.m_id = Convert.ToInt32(sid);
            tmp.parseGold(node.Attributes["goldPlayer"].Value);
            tmp.parsePay(node.Attributes["payPlayer"].Value);
    ...
    // 获取金币字段总个数
    // channelId 1表示闲玩 2蛋蛋赚
    // qihao 期号
    // playerType玩家类型
    public int getChannelParamGoldCount(int channelId, int qihao)
    {
        int id = channelId * 10000 + qihao;
        var data = getValue(id);
        if (data == null) return 0;

        return data.m_goldPlayer.Count;
    }

    // 获取金币对应索引处的值
    public ChannelParam getChannelParamGold(int channelId, int qihao, int index)
    {
        int id = channelId * 10000 + qihao;
        var data = getValue(id);
        if (data == null) return null;

        return data.m_goldPlayer[index];
    }

    //获取充值字段总个数
    public int getChannelParamPayCount(int channelId, int qihao)
    {
        int id = channelId * 10000 + qihao;
        var data = getValue(id);
        if (data == null) return 0;

        return data.m_payPlayer.Count;
    }

    // 获取充值对应索引处的值
    public ChannelParam getChannelParamPay(int channelId, int qihao, int index)
    {
        int id = channelId * 10000 + qihao;
        var data = getValue(id);
        if (data == null) return null;

        return data.m_payPlayer[index];
    }
}

///////////////////////////////////////////////////////////////////////////
public class FishRoomCFG : XmlDataTable<FishRoomCFG, int, ItemCFGData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "Fish_RoomCFG.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
        ...
        {
            ItemCFGData tmp = new ItemCFGData();
            string sid = node.Attributes["RoomID"].Value;
            int roomType = Convert.ToInt32(node.Attributes["RoomType"].Value);
            tmp.m_itemId = Convert.ToInt32(sid);
            tmp.m_itemName = node.Attributes["RedPackage"].Value;
            if (!string.IsNullOrEmpty(tmp.m_itemName))
            {
                int count = 0;
                if(int.TryParse(tmp.m_itemName, out count) && count > 0)
                    m_data.Add(tmp.m_itemId, tmp);
            }
        }
    }
...
//CPL渠道
public class TdChannelCPL : XmlDataTable<TdChannelCPL, string, TdChannelInfo>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "td_channel_cpl.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
...
//WEB渠道
public class TdChannelWEB : XmlDataTable<TdChannelWEB, string, TdChannelInfo>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "td_channel_web.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
...
public class ExchangeCfg : XmlDataTable<ExchangeCfg, int, ExchangeData>, IXmlData
{
    public void init()
    {
        ...
        for (node = node.FirstChild; node != null; node = node.NextSibling)
        {
            ...
            tmp.m_managementName = node.Attributes["ManageMentName"].Value;
            tmp.m_type = Convert.ToInt32(node.Attributes["ManageMent"].Value);
            m_data.Add(tmp.m_id, tmp);
        }
    }
}

public class RedpackageExchangeCfg : XmlDataTable<RedpackageExchangeCfg, int, ExchangeData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "M_RedPackageExchangeCFG.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
...
public class Activ7DayRechargeExchangeCfg : XmlDataTable<Activ7DayRechargeExchangeCfg, int, ExchangeData>, IXmlData
{
    public void init()
    {
        string path = HttpRuntime.BinDirectory + "..\\" + "data";
        string file = Path.Combine(path, "M_ActivSevDayReExchangeCFG.xml");
        XmlDocument doc = new XmlDocument();
        doc.Load(file);
...
public class ChannelInfo100003
{
    // ID_GOLD  ID_MONEY
    public int m_id; // 金币还是钻钻石

    public long m_count; // 具体数额
...
public class StatChannel100003_CFG : XmlDataTable<StatChannel100003_CFG, int, StatChannel100003_CFGData>, IXmlData
{
    ...
    // dbKey 如 pay0, gold0
    // 拆分信息
    public ChannelInfo100003 splitInfo(string dbKey)
    {
        ChannelInfo100003 info = new ChannelInfo100003();
        if (dbKey.StartsWith("pay"))
        {
            info.m_id = ID_MONEY;
            info.m_index = Convert.ToInt32(dbKey.Substring(3));
            var d = getValue(info.m_index);
            if (d != null)
            {
                info.m_count = d.m_dataList[info.m_index];
            }
            else
            {
                info.m_count = 0;
        ...
        else if (dbKey.StartsWith("gold"))
        {
            info.m_id = ID_GOLD;
            info.m_index = Convert.ToInt32(dbKey.Substring(4));
            var d = getValue(info.m_index);
            if (d != null)
            {
                info.m_count = d.m_dataList[info.m_index];
