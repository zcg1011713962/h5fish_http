...
public class DBMgr : CDBMgr<DBMgr>
{
    // 初始化
    public override void open()
    {
        ...
        // 登陆后台的账号数据库,登陆游戏的账号数据库 + 充值数据库 + 游戏数据库
        m_mongoServer = new CDBServer[count + 2];
        // 初始化账号及操作日志所在的dbserver
        m_mongoServer[0] = create(WebConfigurationManager.AppSettings["account"] + "_account", null, true);
        m_dbServer.Add(WebConfigurationManager.AppSettings["account"] + "_account", 0);
    ...
    // 返回指定数据库所在的服务器ID
    public override int getSpecialServerId(int dbid)
    {
        switch (dbid)
        {
            ...
            case DbName.DB_PAYMENT:
                {
                    ...
                    return getDbId(dbIP + "_account", true);
                }
            default:
                break;
        }
    ...
    public override DbServerInfo getDbInfo(string url) 
    {
        DbServerInfo dbInfo = ResMgr.getInstance().getDbInfo(url);
        return dbInfo;
    }

    public override bool isAccountDb(string url) 
    {
        return url == WebConfigurationManager.AppSettings["account"] + "_account";
    }

    public override string getAccountDbURL()
    {
        return WebConfigurationManager.AppSettings["account"];
    }
...
//         if (!m_dbServer.ContainsKey(WebConfigurationManager.AppSettings["payment"] + "_payment"))
//         {
//             m_dbServer.Add(WebConfigurationManager.AppSettings["payment"], i++);
//         }
    }
...
//                     return getDbId(dbIP + "_payment", true);
//                 }
            default:
                break;
        }
    ...
    public bool save(string tablename, Dictionary<string, object> data, string filedname, object val, DbInfoParam dbParam)
    {
        ...
        try
        {
            MongodbHelper db = m_mongoServer[dbParam.ServerId].getDB(dbParam.DbName, dbParam.ServerType);
...
public class DBServer
{
    ...
    private MongodbHelper[] m_clientSlave = null;
    ...
    public bool init(string url, DbServerInfo dbInfo, bool special = false)
    {
        ...
        m_clientSlave = new MongodbHelper[DbName.DB_NAME_MAX];
...
//         }
    ...
    public MongodbHelper getDB(int index, int serverType = DbInfoParam.SERVER_TYPE_MASTER)
    {
    ...
    void createPLDB(MongodbHelper[] client, string playerURL, string logURL)
    {
        client[DbName.DB_PLAYER] = new MongodbHelper();
        client[DbName.DB_PLAYER].MongoDb = create(m_dbName[DbName.DB_PLAYER], playerURL);

        client[DbName.DB_PUMP] = new MongodbHelper();
        client[DbName.DB_PUMP].MongoDb = create(m_dbName[DbName.DB_PUMP], logURL);

        client[DbName.DB_GAME] = new MongodbHelper();
