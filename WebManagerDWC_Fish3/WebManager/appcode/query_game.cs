...
public class ResultFishBoss
{
    ...
    public int m_totalReleaseDimensity;//放出魔石
    ...
    public long m_totalReleaseGold;//Boss放出金币
...
{
    static int[] s_boss = { 21, 701, 702, 703, 704, 705, 706, 708, 709, 710, 711};
    ...
    public override OpRes doQuery(object param, GMUser user)
    {
        ...
        {
            //遍历多个
            IMongoQuery[] queries = new IMongoQuery[s_boss.Length];
            int k=0;
            foreach (int bossId in s_boss) 
            {
               queries[k] = Query.EQ("bossId", bossId);
                k++;
            }
            imq4 = Query.Or(queries);
        }
    ...
    private OpRes query(GMUser user, IMongoQuery imq)
    {
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            //BOSS放出魔石
            if (data.ContainsKey("totalReleaseDimensity"))
                info.m_totalReleaseDimensity = Convert.ToInt32(data["totalReleaseDimensity"]);
            ...
            if (data.ContainsKey("totalReleaseGold")) //BOSS放出金币
                info.m_totalReleaseGold = Convert.ToInt64(data["totalReleaseGold"]);
...
///////////////////////////////////////////////////////////////
public class FishArenaRankInfo
{
    public string m_nickName;  // 玩家名称
    public int m_playerId;     // 玩家ID
    public int m_socre;        // 积分
    public int m_rank;
}

public class FishArenaRankList
{
    public List<FishArenaRankInfo> m_rank = new List<FishArenaRankInfo>();

    public DateTime m_time;
}

public class FishArenaStat
{
    // 日期
    public DateTime m_time;
    // 钻石收入
    public int m_gemIncome;
    // 参赛人数
    public int m_joinPerson;
    // 参赛人次
    public int m_joinPersonCount;
}

public class FishArenaRankWrap
{
    public Dictionary<DateTime, FishArenaRankList> m_dataDic = new Dictionary<DateTime, FishArenaRankList>();

    public FishArenaRankList m_dataLine = new FishArenaRankList();

    public List<FishArenaStat> m_stat = new List<FishArenaStat>();

    public void addRankCur(FishArenaRankInfo info)
    {
        if (info != null)
        {
    ...
    public void addRankHistory(DateTime time, FishArenaRankInfo info)
    {
        FishArenaRankList obj = null;
...
// 竞技场查询
public class QueryFishArena : QueryBase
{
    // 竞技场当前日榜
    public const int RANK_DAY_CUR = 1;
    // 竞技场历史日榜
    public const int RANK_DAY_HISTORY = 2;
    // 竞技场当前周榜
    public const int RANK_WEEK_CUR = 3;
    // 竞技场历史周榜
    public const int RANK_WEEK_HISTORY = 4;
    // 竞技场统计
    public const int ARENA_STAT = 5;
    // 自由赛当日榜
    public const int FREE_RANK_CUR = 6;
    // 自由赛历史榜
    public const int FREE_RANK_HISTORY = 7;
    ...
    private OpRes query(GMUser user, ParamQuery p, IMongoQuery imq)
    {
        switch (p.m_op)
        {
            case RANK_DAY_CUR: // 当前日榜
                {
                    queryRankDayCur(user, p, imq);
                }
                break;
            case RANK_DAY_HISTORY: // 历史日榜
                {
                    queryRankDayHistory(user, p, imq);
                }
                break;
            case RANK_WEEK_CUR: // 当周榜
                {
                    queryRankWeekCur(user, p, imq);
                }
                break;
            case RANK_WEEK_HISTORY: // 历史周榜
                {
                    queryRankWeekHistory(user, p, imq);
                }
                break;
            case ARENA_STAT:
                {
                    queryArenaStat(user, p, imq);
                }
                break;
            default:
                return OpRes.op_res_failed;
    ...
    OpRes constructCond(ParamQuery p, ref IMongoQuery imq)
    {
        ...
        if (p.m_op == RANK_DAY_HISTORY ||
            p.m_op == RANK_WEEK_HISTORY)
        {
    ...
    OpRes queryRankDayCur(GMUser user, ParamQuery p, IMongoQuery imq)
    {
        ...
        IMongoQuery timq = Query.GT("dayPoints", 0);
        dataList = DBMgr.getInstance().executeQuery(TableName.ARENA_MATCH_PLAYER, dip, timq, 0, 100, null, "dayPoints", false);
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            if (data.ContainsKey("dayPoints"))
            {
                tmp.m_socre = Convert.ToInt32(data["dayPoints"]);
            }
            tmp.m_rank = i + 1;
    ...
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(), DbName.DB_GAME, DbInfoParam.SERVER_TYPE_SLAVE);
        user.totalRecord = DBMgr.getInstance().getRecordCount(TableName.ARENA_RANK_DAY_HISTORY, imq, dip);

        SortByBuilder sort = new SortByBuilder();
        sort = sort.Descending("genTime").Ascending("rank");

        List<Dictionary<string, object>> dataList = null;
        dataList = DBMgr.getInstance().executeQuery2(TableName.ARENA_RANK_DAY_HISTORY, dip, imq,
    ...
    OpRes queryRankWeekCur(GMUser user, ParamQuery p, IMongoQuery imq)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(), DbName.DB_GAME, DbInfoParam.SERVER_TYPE_SLAVE);
        List<Dictionary<string, object>> dataList = null;
        IMongoQuery timq = Query.GT("weekPoints", 0);
        dataList = DBMgr.getInstance().executeQuery(TableName.ARENA_MATCH_PLAYER, dip, timq, 0, 100, null, "weekPoints", false);
        if (dataList == null) return OpRes.op_res_not_found_data;
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            if (data.ContainsKey("weekPoints"))
            {
                tmp.m_socre = Convert.ToInt32(data["weekPoints"]);
            }
            tmp.m_rank = i + 1;
    ...
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(), DbName.DB_GAME, DbInfoParam.SERVER_TYPE_SLAVE);
        user.totalRecord = DBMgr.getInstance().getRecordCount(TableName.ARENA_RANK_WEEK_HISTORY, imq, dip);

        SortByBuilder sort = new SortByBuilder();
        sort = sort.Descending("genTime").Ascending("rank");

        List<Dictionary<string, object>> dataList = null;
        dataList = DBMgr.getInstance().executeQuery2(TableName.ARENA_RANK_WEEK_HISTORY, dip, imq,
            (param.m_curPage - 1) * param.m_countEachPage,
            param.m_countEachPage, null, sort);
    ...
    // 竞技场统计
    OpRes queryArenaStat(GMUser user, ParamQuery param, IMongoQuery imq)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(), DbName.DB_PUMP, DbInfoParam.SERVER_TYPE_SLAVE);
        ...
        List<Dictionary<string, object>> dataList = null;
        dataList = DBMgr.getInstance().executeQuery2(TableName.PUMP_ARENA_STAT, dip, imq,
            0,
            0, null, sort);
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            IMongoQuery timq = Query.EQ("genTime", tmp.m_time);
            tmp.m_joinPerson = (int)DBMgr.getInstance().getRecordCount(TableName.PUMP_ARENA_STAT_DETAIL, timq, dip);
    ...
    OpRes queryFreeRankDayCur(GMUser user, ParamQuery p, IMongoQuery imq)
    {
        ...
        IMongoQuery timq = Query.And(Query.GT("dayPoints", 0), imq1);

        dataList = DBMgr.getInstance().executeQuery(TableName.ARENA_FREE_MATCH_PLAYER, dip, timq, 0, 100, null, "dayPoints", false);
        if (dataList == null) return OpRes.op_res_not_found_data;
