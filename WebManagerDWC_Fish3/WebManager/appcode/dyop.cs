...
public enum DyOpType
{
    ...
    opTypePlayerBasicInfoChange, //修改玩家基本信息
...
class DyOpMgr : SysBase
{
	...
	public override void initSys()
	{
        ...
        m_items.Add(DyOpType.opTypeOperationActivityCFGNew, new DyOpOperationActivityCFGNew());//活动时间设置

        m_items.Add(DyOpType.opTypePlayerBasicInfoChange, new DyOpPlayerBasicInfoChg()); //修改玩家基本信息
	}
}
...
public class DyOpGiftCodeNew : DyOpBase 
{
    public override OpRes doDyop(object param, GMUser user)
    {
        ...
        int giftId = Convert.ToInt32(p.m_giftId);
        ...
        if (giftId <= 0)
        {
            OpRes c = parseSpecialItem(p, out var mailItem);
            if (c != OpRes.opres_success) return c;

            data.Add("items", mailItem);
        }
    ...
    OpRes parseSpecialItem(ParamCDKEY param, out BsonDocument mailItem)
    {
        mailItem = null;
        List<ParamItem> tmpItem = new List<ParamItem>();

        bool res = Tool.parseItemList(param.m_itemList, tmpItem, false);
        if (res)
        {
            for (int i = 0; i < tmpItem.Count; i++)
            {
                var t = ItemCFG.getInstance().getValue(tmpItem[i].m_itemId);
                if (t == null)
...
//活动时间设置
public class DyOpOperationActivityCFGNew : DyOpBase
{
    public override OpRes doDyop(object param, GMUser user)
    {
        ParamQuery p = (ParamQuery)param;
        ...
        if ((p.m_op == 1 || p.m_op == 2) && string.IsNullOrEmpty(p.m_playerId))
            return OpRes.op_res_param_not_valid;
        ...
        if (p.m_op == 2) //删除
        {
            res = DBMgr.getInstance().remove(TableName.GLOBAL_ACTIVITY_CFG, "_id", new ObjectId(p.m_playerId), serverId, DbName.DB_CONFIG);
        }
        else
        {
            if (string.IsNullOrEmpty(p.m_param))
            ...
            //正则表达式判断基本格式  单日期07-06 或者双日期  2020-07-06, 2020-09-12
            Match match = Regex.Match(p.m_param, Exp.DATE_TIME6);
            if (!match.Success)
                return OpRes.op_res_param_not_valid;

            string time_start = "", time_end = "";
            if (p.m_param.IndexOf(',') == -1) // 单日期  2020-07-06
            {
                time_start = p.m_param;
                time_end = p.m_param;
            }
            else  //时间段 2020-07-06, 2020-09-12
            {
                time_start = p.m_param.Trim().Split(',')[0];
                time_end = p.m_param.Trim().Split(',')[1];
            }

            //转为时间格式
            DateTime start_time = Convert.ToDateTime(time_start);
            DateTime end_time = Convert.ToDateTime(time_end);

            //开始时间不大于结束时间
            if (DateTime.Compare(start_time, end_time) > 0)
                return OpRes.op_res_param_not_valid;

            //如果活动时间有重叠，则旧的配置失效，配置新的   否则 加入
            IMongoQuery imq = Query.EQ("activityIds", p.m_type);

            //start_time <= startTime  &&  endTime <= end_time
            IMongoQuery imq1 = Query.And(Query.GTE("startTime", start_time), Query.LTE("endTime", end_time.AddDays(1).AddSeconds(-1)));  

            //start_time <= startTime && end_time >= startTime
            IMongoQuery imq2 = Query.And(Query.GTE("startTime", start_time), Query.LTE("startTime", end_time));

            //start_time <= endTime  && end_time >= endTime
            IMongoQuery imq3 = Query.And(Query.GTE("endTime", start_time.AddDays(1).AddSeconds(-1)), Query.LTE("endTime", end_time.AddDays(1).AddSeconds(-1)));

            //start_time >= startTime && end_time <= endTime 
            IMongoQuery imq4 = Query.And(Query.LTE("startTime", start_time), Query.GTE("endTime", end_time.AddDays(1).AddSeconds(-1)));

            imq =Query.And(imq,  Query.Or(imq1, imq2, imq3, imq4));
            DbInfoParam dip = ItemHelp.createDbParam(serverId, DbName.DB_CONFIG, DbInfoParam.SERVER_TYPE_MASTER);
            List<Dictionary<string, object>> dataList = DBMgr.getInstance().executeQuery(TableName.GLOBAL_ACTIVITY_CFG, dip, imq);
            if (dataList != null && dataList.Count != 0)  //如果存在就删除
            { 
                for (int i=0;  i < dataList.Count; i++) 
                {
                    string _id = Convert.ToString(dataList[i]["_id"]);
                    DBMgr.getInstance().remove(TableName.GLOBAL_ACTIVITY_CFG, "_id", new ObjectId(_id), serverId, DbName.DB_CONFIG);
                }
            }

            Dictionary<string, object> data = new Dictionary<string, object>();
            //for (DateTime date = start_time; DateTime.Compare(date, end_time) <= 0; date = date.AddDays(1))
            {
                data.Clear();
                data.Add("startTime", start_time);
                data.Add("endTime", end_time.AddDays(1).AddSeconds(-1));  //加1天 59分59秒

                //data.Add("date", date);

                if (p.m_op == 0) //新增
                {
                    data.Add("activityIds", p.m_type);
                    res = DBMgr.getInstance().insertData(TableName.GLOBAL_ACTIVITY_CFG, data, serverId, DbName.DB_CONFIG);
                }
                else
                {
                    res = DBMgr.getInstance().update(TableName.GLOBAL_ACTIVITY_CFG, data, "_id", new ObjectId(p.m_playerId), serverId, DbName.DB_CONFIG);
                }
            }
        }
        return res ? OpRes.opres_success : OpRes.op_res_failed;
...
////////////////////////////////////////////////////////////////////////////////////
// 修改玩家基本信息
public class DyOpPlayerBasicInfoChg : DyOpBase
{
    public override OpRes doDyop(object param, GMUser user)
    {
        ParamQuery p = (ParamQuery)param;
        ...
        if (p.m_op == 1)      //修改昵称
        {
            res = DBMgr.getInstance().keyExists(TableName.PLAYER_INFO, "player_id", Convert.ToInt32(p.m_playerId), user.getDbServerID(), DbName.DB_PLAYER);
            if (!res)
                return OpRes.op_res_player_not_exist;

             data.Add("nickname", p.m_param);
            res = DBMgr.getInstance().update(TableName.PLAYER_INFO, data, "player_id", Convert.ToInt32(p.m_playerId), user.getDbServerID(), DbName.DB_PLAYER);
        }
        else if(p.m_op == 2) //修改贡献值
        {
            double contributionVal = 0;
            if (!double.TryParse(p.m_param, out contributionVal))
                return OpRes.op_res_param_not_valid;

            res = DBMgr.getInstance().keyExists(TableName.FISHLORD_PLAYER, "player_id", Convert.ToInt32(p.m_playerId), user.getDbServerID(), DbName.DB_GAME);
            if (!res)
                return OpRes.op_res_player_not_exist;

            data.Add("ContributionValue", (double)contributionVal);
            res = DBMgr.getInstance().update(TableName.FISHLORD_PLAYER, data, "player_id", Convert.ToInt32(p.m_playerId), user.getDbServerID(), DbName.DB_GAME);
        }

        if(res)
            OpLogMgr.getInstance().addLog(LogType.LOG_TYPE_PLAYER_BASIC_INFO_CHG, new LogPlayerBasicInfoChg(p.m_op, p.m_playerId, p.m_param), user);
...
public class DyOpFasterStartForVisitor : DyOpBase
{
    ...
    public override OpRes doDyop(object param, GMUser user)
    {
        ...
        if (p.m_optype == 1)
        {
            return oldNewPlayerIdModify(user, p.m_playerId, p.m_account, serverId);
        }
        ...
        bool res = false;
        string pwd = Tool.getMD5Hash(p.m_pwd + DefCC.PWD_EXT);
    ...
    class NOInfo
    {
        public int m_oldId;
        public int m_newId;
        public string m_oldAcc;
        public string m_newAcc;
        public string m_oldPlatform;
        public string m_newPlatform;
    }

    OpRes oldNewPlayerIdModify(GMUser user, string oldPlayerId, string newPlayerId, int serverId)
    {
        OpRes res = getONInfo(user, oldPlayerId, newPlayerId, out NOInfo info);
        if (res != OpRes.opres_success)
            return res;

        string tableName = info.m_newPlatform + "_acc";
        Dictionary<string, object> newAccount = DBMgr.getInstance().getTableData(tableName, "acc_real", info.m_newAcc, serverId, DbName.DB_ACCOUNT);
        if (newAccount == null)
        {
            CLOG.Info("DyOpFasterStartForVisitor.oldNewPlayerIdModify, not find acc {0}", info.m_newAcc);
            return OpRes.opres_success;
        }
        if (!newAccount.ContainsKey("acc"))
        {
            CLOG.Info("DyOpFasterStartForVisitor.oldNewPlayerIdModify, no field acc");
            return OpRes.op_res_failed;
        }

        Dictionary<string, object> updata = new Dictionary<string, object>();
        updata.Add("acc_real", info.m_oldAcc);
        string acc = Convert.ToString(newAccount["acc"]);
        bool resOp = DBMgr.getInstance().update(tableName, updata, "acc", acc, serverId, DbName.DB_ACCOUNT);
        if(resOp)
        {
            OpLogMgr.getInstance().addLog(LogType.LOG_TYPE_NEW_OLD_ACC,
               new LogOldNewAcc(info.m_oldId, info.m_newId),
               user,
               "");
        }
        return resOp ? OpRes.opres_success : OpRes.op_res_failed;
    }

    OpRes getONInfo(GMUser user, string oldPlayerId, string newPlayerId, out NOInfo outInfo)
    {
        outInfo = null;

        int oldId = 0;
        int newId = 0;
        if (!int.TryParse(oldPlayerId, out oldId))
            return OpRes.op_res_param_not_valid;

        if (!int.TryParse(newPlayerId, out newId))
            return OpRes.op_res_param_not_valid;

        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(), DbName.DB_PLAYER, DbInfoParam.SERVER_TYPE_SLAVE);
        IMongoQuery imqOld = Query.EQ("player_id", oldId);
        IMongoQuery imqNew = Query.EQ("player_id", newId);
        Dictionary<string, object> oldData = DBMgr.getInstance().getTableData(TableName.PLAYER_INFO, dip, imqOld, s_fields);
        if (oldData == null)
            return OpRes.op_res_player_not_exist;

        Dictionary<string, object> newData = DBMgr.getInstance().getTableData(TableName.PLAYER_INFO, dip, imqNew, s_fields);
        if (newData == null)
            return OpRes.op_res_player_not_exist;

        if (!oldData.ContainsKey("account") || !oldData.ContainsKey("account") ||
            !newData.ContainsKey("platform") || !newData.ContainsKey("platform"))
        {
            CLOG.Info("DyOpFasterStartForVisitor.getONInfo not find acc or platform");
            return OpRes.op_res_failed;
        }

        NOInfo info = new NOInfo();
        info.m_oldId = oldId;
        info.m_newId = newId;
        info.m_newAcc = Convert.ToString(newData["account"]);
        info.m_oldAcc = Convert.ToString(oldData["account"]);
        info.m_newPlatform = Convert.ToString(newData["platform"]);
        info.m_oldPlatform = Convert.ToString(oldData["platform"]);
        if(info.m_oldPlatform != info.m_newPlatform)
        {
            CLOG.Info("DyOpFasterStartForVisitor.getONInfo accReal {0} {1} platform not same", info.m_oldAcc, info.m_newAcc);
            return OpRes.op_res_failed;
        }
        outInfo = info;
        return OpRes.opres_success;
...
public class DyOpSendMail : DyOpBase
{
    ...
    private OpRes fullSend(ParamSendMail p, GMUser user, int days)
    {
        ...
        if (!string.IsNullOrEmpty(p.m_channelId))
        {
            if (!int.TryParse(p.m_channelId, out var c))
            {
                return OpRes.op_res_param_not_valid;
            }
            param.m_condition.Add("channel", p.m_channelId);
        }
...
public class DyOpExchange : DyOpBase
{
    ...
    {
        {1002, 30}, {1003, 50}, {1004, 100}, {1025, 50}, { 1028, 200},
        { 1027,50}, { 1153,50}
    };
...
public class ParamFishlord5 : ParamFishlordParamAdjust
{
    ...
    //抽水率
    public string m_bigPoolPumpRate;
    //期望盈利率
    public string m_bigPoolExpectedEarnRate;
    ...
    public string m_colorFishCtrCount1;
    public string m_colorFishCtrCount2;

    public string m_colorFishCtrRate1;
    public string m_colorFishCtrRate2;
}
...
public class DyOpFishlordParamAdjustNew : DyOpParamAdjust
{
    ...
    protected override OpRes modifyExp(GMUser user, ParamFishlordParamAdjust p)
    {
        ...
        if (param.m_isNewAlg)
        {
            ...
            if (code == OpRes.opres_success)
            {
                ...
                updata.Add("BigPoolPumpRate", result.m_bigPoolPumpRate);
                updata.Add("BigPoolExpectEarnRate", result.m_bigPoolExpectedEarnRate);

                if (result.m_roomId == 2) {
                    updata.Add("ColorFishCtrCount1", result.m_colorFishCtrCount1);
                    updata.Add("ColorFishCtrCount2", result.m_colorFishCtrCount2);
                    updata.Add("ColorFishCtrRate1", result.m_colorFishCtrRate1);
                    updata.Add("ColorFishCtrRate2", result.m_colorFishCtrRate2);
                }
                ...
                OpLogMgr.getInstance().addLog(LogType.LOG_TYPE_MODIFY_FISHLORD_CTRL_PARAM,
                        new LogModifyFishlordCtrlNewParam(roomId, result.m_jsckpotGrandPump, result.m_jsckpotSmallPump, result.m_normalFishRoomPoolPumpParam, result.m_baseRate,
                            result.m_checkRate, result.m_trickDeviationFix, result.m_incomeThreshold, result.m_bigPoolPumpRate,
                            result.m_bigPoolExpectedEarnRate,
                            //result.m_legendaryFishRate,
                            result.m_mythicalScoreTurnRate, result.m_mythicalFishRate,
                            result.m_colorFishCtrCount1, result.m_colorFishCtrCount2, result.m_colorFishCtrRate1, result.m_colorFishCtrRate2), user);
            }
    ...
    OpRes calParamInner(ParamFishlord5 param, ParamInner result)
    {
        ...
        //抽水率
        if(!int.TryParse(param.m_bigPoolPumpRate, out result.m_bigPoolPumpRate) || !int.TryParse(param.m_bigPoolExpectedEarnRate, out result.m_bigPoolExpectedEarnRate))
            return OpRes.op_res_param_not_valid;

        if(result.m_bigPoolPumpRate < 0 || result.m_bigPoolExpectedEarnRate < 0)
            return OpRes.op_res_param_not_valid;
        ...
        /////////////////////////////////////
        if (result.m_roomId == 2 || result.m_roomId == 3 || result.m_roomId == 5 || result.m_roomId == 6 || result.m_roomId == 7 || result.m_roomId == 10)
        {
        ...
        if (result.m_roomId == 2) {
            if (!int.TryParse(param.m_colorFishCtrCount1, out result.m_colorFishCtrCount1))
                return OpRes.op_res_param_not_valid;

            if (!int.TryParse(param.m_colorFishCtrCount2, out result.m_colorFishCtrCount2))
                return OpRes.op_res_param_not_valid;

            if (!int.TryParse(param.m_colorFishCtrRate1, out result.m_colorFishCtrRate1))
                return OpRes.op_res_param_not_valid;

            if (!int.TryParse(param.m_colorFishCtrRate2, out result.m_colorFishCtrRate2))
                return OpRes.op_res_param_not_valid;

            if (result.m_colorFishCtrCount1 < 0 || result.m_colorFishCtrCount2 < 0 ||
                result.m_colorFishCtrRate1 < 0 || result.m_colorFishCtrRate1 > 100 ||
                result.m_colorFishCtrRate2 < 0 || result.m_colorFishCtrRate2 > 100  )
                return OpRes.op_res_param_not_valid;
        }
...
public class DyOpOperationGameCtrl : DyOpBase
{
    ...
    public class DyOpKickPlayer : DyOpBase
    {
        ...
        public override OpRes doDyop(object param, GMUser user)
        {
                ...
                case "dec":
                    {
                        ...
                        if (res == OpRes.opres_success)
                        {
                            ...
                            else if (p.m_prop == "rechargeContr")
                            {
                                data.Add("rtype", (int)RechargeType.playerRechargeContri);
                            }
            ...
            case "modify":
                {
                    if(p.m_prop == "nickName")
                    {
                        CommandModifyNickName cmd = new CommandModifyNickName();
                        CMemoryBuffer buf = new CMemoryBuffer();
                        buf.Writer.Write(p.m_acc);
                        buf.Writer.Write(p.m_value);
                        res = cmd.execute(buf, user);
                    }
                }
                break;
            }
