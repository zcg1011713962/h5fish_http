...
class StatMgr : SysBase
{
    ...
    public override void initSys()
    {
        ...
        m_items.Add(StatType.statTypeRechargeNew, new StatRechargeNew());   //充值统计

        m_items.Add(StatType.statTypeSameOrderId, new StatSameOrderId());
        m_items.Add(StatType.statTypeActiveCount, new StatActiveCount());
        m_items.Add(StatType.statTypeActivePerson, new StatActivePerson());  //活跃人数
        m_items.Add(StatType.statTypeVipLevel, new StatVipLevel());

        m_items.Add(StatType.statTypeLTV, new StatLTV()); //LTV全部平均
...
public class StatLTV : PumpBase
{
    ...
    public override OpRes doStat(object param, GMUser user)
    {
        ...
        else if (p.m_param == "-4")  //APP总体   总体 - 微信/QQ - cpl总体
        {
            //排除  微信/QQ
            var channelList = TdChannelExcept.getInstance().getAllData();
            if (channelList != null)
            ...
            // 排除 CPL总体
            var channelList2 = TdChannelCPL.getInstance().getAllData();
        ...
        else if (p.m_param == "-3") //CPL总体
        {
            ...
            if (channelList != null)
            {
                ...
                imq = Query.And(imq, Query.Or(queries));
            }
        }
        else if (p.m_param == "-2") // WEB总体
        {
