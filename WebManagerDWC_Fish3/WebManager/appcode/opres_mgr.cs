...
public enum OpRes
{
    ...
    op_res_third_part_platform,     // 这是第三方平台，无法完成操作
    op_res_no_right,                // 没有权限操作
    op_res_not_join_match,          // 还没有参加比赛
    op_res_score_lower,             // 不能比当前成绩低
    op_res_not_exist_in_two_list,   //不能同时在两个名单中
    op_res_not_in_range,            //时间范围太大（不大于一个月）
    op_res_not_visitor,             //玩家非游客
    op_res_has_set_account,         //账号已设置
    op_res_has_exist,               //账号已占用
    op_res_has_default_acc,         //已有默认账号
    op_res_has_exist_id,            //已存在ID
    op_res_has_bind, 
    op_res_player_beyond_limit,     //人数超出限定额
    op_res_not_robot,
}
