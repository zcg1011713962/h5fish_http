$(document).ready(function () {
    ...
    function viewBuff(jsonObj) {
        $('#buffPlayerList').html('');
        var head = '<tr><td>玩家ID</td><td>玩家昵称</td><td>水池类型</td><td>BUFF水池上次操作时间</td><td>BUFF水池当前操作值</td><td>BUFF水池当前值</td><td>到期日期</td><td>操作</td></tr>';
        ...
        for (var i = 0; i < bList.length; i++) {
            ...
            BTN_ID++;
            var tstr = '<tr id="' + obj.m_id + '" moneyType="' + obj.m_moneyType + '">' +
                '<td>' + obj.m_playerId + '</td>' +
                '<td>' + obj.m_nickName + '</td>' +
                '<td>' + getMoneyTypeName(obj.m_moneyType) + '</td>' +
                '<td>' + obj.m_lastFixTime + '</td>' +
                '<td>' + obj.m_lastPoolSetVal + '</td>' +
                '<td>' + obj.m_playerScorePool + '</td>' +
                '<td>' + obj.m_scorePoolTimeLeft + '</td>';
            ...
            $('#' + btnId).click(function () {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        "moneyType": $(this).parent().parent().attr("moneyType")
                    },
                    success: function (data) {
                        processRet(data);
                    }
    ...
    function removeBuff(obj) {
        if (obj.result == 0)
            $('#buffPlayerList').find('tr[id=' + obj.m_id + ']').remove();
    }

    function getMoneyTypeName(key) {
        var moneyName = "";
        switch (key) {
            case 1: moneyName = "金币"; break;
            case 19: moneyName = "龙珠碎片"; break;
        }
        return moneyName;
    }
