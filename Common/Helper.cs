...
namespace Common
{
    public class Helper
    {
        ...
        static public string getMD5Upper(string source)
        {
            MD5 md5Service = new MD5CryptoServiceProvider();
            byte[] src = Encoding.UTF8.GetBytes(source);
            byte[] res = md5Service.ComputeHash(src, 0, src.Length);
        ...
        // 结果转成16进制串，结果以小写表示
        static public string SHA1EncryptBy16Hex(string plainText, string strKey, bool isLower = true )
        {
            HMACSHA1 hmacsha1 = new HMACSHA1();
            hmacsha1.Key = Encoding.UTF8.GetBytes(strKey);
            byte[] dataBuffer = Encoding.UTF8.GetBytes(plainText);
            ...
            {
                build.Append(String.Format("{0:X2}", hashBytes[i]));
            }
