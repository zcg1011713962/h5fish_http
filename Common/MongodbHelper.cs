...
{   
    ...
    public string ExecuteUpdateByQuery(string table, IMongoQuery queries, UpdateBuilder ub, UpdateFlags flags = UpdateFlags.Upsert)
    {
        try
        {
            var cb = mMongodbClient.GetCollection(table);
    ...
    public string removeByQuery(string table, IMongoQuery query, ref object obj)
    {
        try
        {
            var cb = mMongodbClient.GetCollection(table);
