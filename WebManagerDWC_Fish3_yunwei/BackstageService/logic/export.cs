...
public class ExportPlayerActivityTurret : ExportExcelBase 
{
    ...
    public override string initExport(ExportParam param, int dbServerId)
    {
        ...
        if (cond != null) 
        {
            ...
            //炮倍 默认全部
            if (cond.ContainsKey("turret_level"))
            {
                //炮倍
                ...
                int turretId = 0;
                if (int.TryParse(turret_lv.Trim(), out turretId)) 
                {
                    lv.Add(turretId);
                    lv.Add(turretId);
                }
                ...
                {
                    Tool.parseNumList(turret_lv, lv);
                }

                //根据炮倍得到等级
                var da1 = Fish_TurretOpenRateCFG.getInstance().getValue(lv[0]);
                var da2 = Fish_TurretOpenRateCFG.getInstance().getValue(lv[1]);

                IMongoQuery lv1 = Query.GTE("turret_level", BsonValue.Create(da1.m_level));
                IMongoQuery lv2 = Query.LTE("turret_level", BsonValue.Create(da2.m_level));
                IMongoQuery lv_imq = Query.And(lv1, lv2);
                queryList.Add(lv_imq);

                fileName += "-" + "炮倍[" + lv[0] + "," + lv[1] + "]";
            }
            else 
            {
                fileName += "-" + "全部炮倍";
        ...
        while (true)
        {
            ...
            for (int i = 0; i < dataList.Count; i++)
            {
                try
                {
                    ...
                    string c = Convert.ToString(data["channel_number"]);
...
public class ExportRechargePlayer : ExportExcelBase
{
    private static string[] s_head = new string[] { "玩家id","渠道", "充值次数", "充值金额", 
        ...
        StrName.getGameName3((int)GameId.crocodile),
        /*StrName.getGameName3((int)GameId.baccarat)
