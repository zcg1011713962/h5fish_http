...
public class StatUnitRemain : StatBase
{
    ...
    // 付费玩家的注册账号
    public const string ACC_RECHARGE = "RegisterLogRecharge";
    ...
    {
        statXDayRemain(param, CRemainConst.DAY_2_REMAIN, ref result.m_2DayRemainCountTmp);
        statXDayDevRemain(param, CRemainConst.DAY_2_REMAIN, ref result.m_2DayDevRemainCountTmp);

        m_queryTable = ACC_RECHARGE;
        statXDayRemain(param, CRemainConst.DAY_2_REMAIN, ref result.m_2DayRemainCountRechargeTmp);
    }

    // 统计3日留存
    private void stat3DayRemain(ParamStat param, StatResult result)
    {
        statXDayRemain(param, CRemainConst.DAY_3_REMAIN, ref result.m_3DayRemainCountTmp);
        statXDayDevRemain(param, CRemainConst.DAY_3_REMAIN, ref result.m_3DayDevRemainCountTmp);

        m_queryTable = ACC_RECHARGE;
        statXDayRemain(param, CRemainConst.DAY_3_REMAIN, ref result.m_3DayRemainCountRechargeTmp);
    }

    // 统计7日留存
    private void stat7DayRemain(ParamStat param, StatResult result)
    {
        statXDayRemain(param, CRemainConst.DAY_7_REMAIN, ref result.m_7DayRemainCountTmp);
        statXDayDevRemain(param, CRemainConst.DAY_7_REMAIN, ref result.m_7DayDevRemainCountTmp);

        m_queryTable = ACC_RECHARGE;
        statXDayRemain(param, CRemainConst.DAY_7_REMAIN, ref result.m_7DayRemainCountRechargeTmp);
    }

    // 统计30日留存
    private void stat30DayRemain(ParamStat param, StatResult result)
    {
        statXDayRemain(param, CRemainConst.DAY_30_REMAIN, ref result.m_30DayRemainCountTmp);
        statXDayDevRemain(param, CRemainConst.DAY_30_REMAIN, ref result.m_30DayDevRemainCountTmp);
    }
    ...
    protected void queryRegeditAcc(ParamStat param, DateTime dt)
    {
        ...
        List<Dictionary<string, object>> dataList =
            MongodbAccount.Instance.ExecuteGetListByQuery(string.IsNullOrEmpty(m_queryTable) ? cinfo.m_regeditTable : m_queryTable,
            imq, new string[] { ACC_KEY });
