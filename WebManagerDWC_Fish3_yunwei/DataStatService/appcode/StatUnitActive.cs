...
public class StatUnitActive : StatBase
{
    public override void doStat(object param, StatResult result)
    {
        ...
        result.m_deviceLoginCount = getDeviceActive(imq, cinfo);
    }

    // 得到设备活跃
    int getDeviceActive(IMongoQuery imq, ChannelInfo cinfo)
    {
        int count = MongodbAccount.Instance.ExecuteDistinct(cinfo.m_loginTable, "acc_dev", imq);
        return count;
    }
}
