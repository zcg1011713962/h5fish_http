...
public class QueryGameTimeDistribution : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        ...
        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.STAT_GAME_TIME_FOR_DISTRIBUTION_RESULT, dip, imq,
...
public class QueryGameTimePlayerFavor : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(),DbName.DB_PUMP,DbInfoParam.SERVER_TYPE_SLAVE);
        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.STAT_GAME_TIME_FOR_PLAYER_FAVOR_RESULT, dip, imq,
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            ...
            //for (int k = 0; k < StrName.s_gameName3.Count; k++)
            for (int k = 0; k < 2; k++)
            {
...
public class PlayerTypeDataCollect<TData> where TData : new()
{
    ...
    public PlayerTypeData<TData> getDataByTime(DateTime dt)
    {
        PlayerTypeData<TData> d = default(PlayerTypeData<TData>);
        if (m_data.ContainsKey(dt))
        {
...
public class QueryFirstRechargeGameTimeDistribution : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(),DbName.DB_PUMP,DbInfoParam.SERVER_TYPE_SLAVE);
        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.STAT_FIRST_RECHARGE_GAME_TIME_DISTRIBUTION_RESULT, dip, imq, 0, 0, null, "genTime", true);
...
public class QueryFirstRechargePointDistribution : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(),DbName.DB_PUMP,DbInfoParam.SERVER_TYPE_SLAVE);
        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.STAT_FIRST_RECHARGE_POINT_DISTRIBUTION_RESULT, dip, imq, 0, 0, null, "genTime", true);
...
public class QueryPlayerGameBet : QueryBase
{
    ...
    private OpRes query(ParamQuery param, IMongoQuery imq, GMUser user)
    {
        DbInfoParam dip = ItemHelp.createDbParam(user.getDbServerID(),DbName.DB_PUMP,DbInfoParam.SERVER_TYPE_SLAVE);
        List<Dictionary<string, object>> dataList =
             DBMgr.getInstance().executeQuery(TableName.STAT_PLAYER_GAME_BET_RESULT, dip, imq,
             0, 0, null, "genTime", false);
